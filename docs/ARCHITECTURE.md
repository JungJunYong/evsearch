...
조회 주기는 사용자 설정이 아니다. 서버가 **회차마다 30~60초 무작위 간격**으로 조회해
실시간성과 과호출 방지를 함께 맞추고, 구독별 시점이 겹치지 않게 한다.

서버 조회는 **알림을 켰고 감시 시간 범위 안일 때만** 돈다. 알림을 끄면 앱이 구독을
해지하므로 서버는 아무 조회도 하지 않는다(그 구간의 갱신은 앱의 15분 주기와 수동
새로고침이 담당한다).

푸시는 **이전 관측과 달라졌을 때만** 나간다. `충전 중 → 충전 가능` 전환은 알림 푸시,
그 밖의 상태 변화는 데이터 전용 푸시(`type=widget_sync`)로 위젯만 갱신한다. 첫 관측은
기준선으로만 쓴다.
...
| 경로 | 조건 | 동작 | 실효 지연 |
|---|---|---|---|
| 서버 상태 변화 감지 | 알림 ON + 시간 범위 내 | BFF가 30~60초 무작위 간격으로 조회 → **변화가 있을 때만** 데이터 전용 푸시 → 앱이 즉시 동기화 후 위젯 재작성 | 수 초 |
| 빈자리 전환 | 알림 ON + 시간 범위 내 | 알림 푸시(`type=vacancy`, data 동봉) → 알림 표시 + 위젯 갱신 | 수 초 |
| 고정 주기 | 항상 | 15분 PeriodicWork 1개 (+ `updatePeriodMillis` 안전망) | 최대 15~30분 |
| 수동 | 사용자 조작 | 위젯 새로고침 탭, 목록 화면 새로고침, 앱 진입/복귀 | 즉시 |

클라이언트 주기는 **15분 고정**이다. 사용자가 고를 수 있는 갱신 주기는 두지 않는다.
실시간성은 서버 푸시가 담당하고, 주기 작업은 푸시가 막혔을 때의 보조 수단이다.

앱은 배치 상태 조회 시 허용 캐시 나이(`maxAgeMs`)를 함께 보낸다. 주기 갱신은 20초,
수동 새로고침은 0초, 서버 폴링은 최소 간격의 절반(15초)을 써서 서버 캐시가 실시간성을
깎지 않게 한다.

**데이터 출처는 BFF(evsearch.wiqio.com) 하나뿐이다.** 앱에 번들된 오프라인 목업
데이터셋(`assets/mockStations.json`)과 그 폴백 경로는 제거했다. 조회가 실패하면 화면은
오류를 표시하고, 위젯/목록은 Room의 마지막 정상 상태를 그대로 유지한다(값을 만들지 않는다).
...
| `POST /v1/alerts/subscribe` | FCM 토큰 + 감시 대상(`keys[].notify`) + 시간 범위 등록 (조회 주기는 서버가 30~60초 무작위로 정함) |
